...
export default function Home() {
  ...
  const [routeAdvisories, setRouteAdvisories] = useState<string[]>([]); // new state
  ...
  const handleReset = () => {
    ...
    setEstimatedArrival('');
    setWeatherAlerts([]);
    setRouteAdvisories([]); // reset advisories
    setError('');
    setLoading(false);
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white shadow-lg rounded-2xl p-4">
            ...
            <Map
              ...
              onAdvisoriesUpdate={adv => setRouteAdvisories(adv)} // new prop
            />
          </div>
              ...
              {weatherAlerts.length > 0 ? (
                <div>
                  ...
                  <ul className="list-disc list-inside text-sm text-red-700">
                    {weatherAlerts.map((a, i) => <li key={i}><strong>{a.event}</strong> <a href={a.link} className="underline text-blue-600" target="_blank" rel="noopener noreferrer">Details</a></li>)}
                  </ul>
                </div>
              ) : null}
              ...
              <div className="mt-4">
                ...
                {routeAdvisories.length > 0 ? (
                  <ul className="list-disc list-inside text-sm">
                    {routeAdvisories.map((text, i) => <li key={i}>{text}</li>)}
                  </ul>
                ) : (
